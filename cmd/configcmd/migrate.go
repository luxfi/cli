...
func newMigrateCmd() *cobra.Command {
	cmd := &cobra.Command{
		Use:          "migrate",
		Short:        "migrate ~/.cli.json and ~/.cli/config to new configuration location ~/.cli/config.json",
		Long:         `migrate command migrates old ~/.cli.json and ~/.cli/config to /.cli/config.json..`,
		RunE:         migrateConfig,
		SilenceUsage: true,
	}
	return cmd
