...
This command currently only supports Subnets deployed on the Testnet and Mainnet.`,
	...
	// Note: testnet, local, mainnet flags are handled by networkoptions.AddNetworkFlagsToCmd in blockchaincmd
...
but until the node is whitelisted, it will not be able to validate this subnet.`)
	...
	if luxConfigPath == "" {
		...
		if luxConfigPath != "" {
			ux.Logger.PrintToUser("Found a config file at %s", luxlog.Bold.Wrap(luxlog.Green.Wrap(luxConfigPath)))
	...
	if pluginDir == "" {
		...
		if pluginDir != "" {
			ux.Logger.PrintToUser("Found the VM plugin directory at %s", luxlog.Bold.Wrap(luxlog.Green.Wrap(pluginDir)))
...
func handleValidatorJoinElasticSubnet(sc models.Sidecar, network models.Network, subnetName string) error {
	...
	txID, err := net.IssueAddPermissionlessValidatorTx(keyChain, subnetID, nodeID, stakedTokenAmount, assetID, uint64(start.Unix()), uint64(endTime.Unix()))
	if err != nil {
		return err
	}
