...
(they relinquish their control keys), specifies an Lux Native Token (ANT) that validators must use for staking 
...
func createAssetID(deployer *net.PublicDeployer,
	maxSupply uint64,
	subnetID ids.ID,
	tokenName string,
	tokenSymbol string,
...
func exportToPChain(deployer *net.PublicDeployer,
	subnetID ids.ID,
...
func importFromXChain(deployer *net.PublicDeployer,
	subnetID ids.ID,
	recipientAddr ids.ShortID,
) (ids.ID, error) {
	owner := &secp256k1fx.OutputOwners{
...
func transformElasticSubnetLocal(sc models.Sidecar, subnetName string, tokenName string, tokenSymbol string, elasticSubnetConfig models.ElasticSubnetConfig) error {
	...
	ux.Logger.PrintToUser("Net Successfully Transformed To Elastic Net!")
...
func handleRemoveAndAddValidators(sc models.Sidecar, subnetID ids.ID, validator ids.NodeID, stakedAmount uint64) error {
	...
	txID, err := net.IssueAddPermissionlessValidatorTx(keyChain, subnetID, validator, stakedAmount, assetID, uint64(startTime.Unix()), uint64(endTime.Unix()))
	if err != nil {
		return err
	}
