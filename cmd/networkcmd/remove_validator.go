...
func removeValidatorSOV(
	...
	ux.Logger.PrintToUser("%s", luxlog.Yellow.Wrap(fmt.Sprintf("Validator manager owner %s pays for the initialization of the validator's removal (Blockchain gas token)", validatorManagerOwner)))
	...
	if force && sc.PoS {
		ux.Logger.PrintToUser("%s", luxlog.Yellow.Wrap(fmt.Sprintf("Forcing removal of %s as it is a PoS bootstrap validator", nodeID)))
	}
