...
import (
	...
	luxSDK "github.com/luxfi/cli/v2/v2/sdk/vm"
...
func PrintHowToApplyConfChangesMessage(blockchainName string) {
	...
	ux.Logger.PrintToUser("  lux network stop")
	ux.Logger.PrintToUser("  lux network start")
	ux.Logger.PrintToUser("")
	ux.Logger.PrintToUser("To apply the change to local validators on Testnet:")
	ux.Logger.PrintToUser("  lux node local stop %s-local-node-testnet", blockchainName)
	ux.Logger.PrintToUser("  lux node local start %s-local-start-testnet", blockchainName)
	ux.Logger.PrintToUser("")
	ux.Logger.PrintToUser("To apply the change to local validators on Mainnet:")
	ux.Logger.PrintToUser("  lux node local stop %s-local-node-mainnet", blockchainName)
	ux.Logger.PrintToUser("  lux node local start %s-local-start-mainnet", blockchainName)
}
...
func ConfigureInitialRewardConfig() (*rewardmanager.InitialRewardConfig, error) {
	...
	option, err := app.Prompt.CaptureList(
		"By default, all transaction fees on Lux are burned (sent to a blackhole address). (Reward Manager Precompile)",
		options,
	)
	if err != nil {
		return config, err
