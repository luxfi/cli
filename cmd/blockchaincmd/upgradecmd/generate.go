...
func PrintHowToApplyConfChangesMessage(blockchainName string) {
	...
	ux.Logger.PrintToUser("  lux node local stop %s-local-node-fuji", blockchainName)
	ux.Logger.PrintToUser("  lux node local start %s-local-start-fuji", blockchainName)
	ux.Logger.PrintToUser("")
