...
var (
	...
	luxdChainConfigDirDefault = filepath.Join("$HOME", ".luxd", "chains")
...
func applyPublicNetworkUpgrade(blockchainName, networkKey string, sc *models.Sidecar) error {
	if print {
		...
		fmt.Println()
		ux.Logger.PrintToUser("1. Identify where your validator has the luxd chain config dir configured.")
		ux.Logger.PrintToUser("   The default is at $HOME/.luxd/chains (%s on this machine).", os.ExpandEnv(luxdChainConfigDirDefault))
	...
	ux.Logger.PrintToUser("Trying to install the upgrade files at the provided %s path", luxdChainConfigDir)
	chainDir := filepath.Join(luxdChainConfigDir, sc.Networks[networkKey].BlockchainID.String())
