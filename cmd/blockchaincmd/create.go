...
func createBlockchainConfig(cmd *cobra.Command, args []string) error {
	...
	ux.Logger.PrintToUser("Run 'lux blockchain describe' to view all created addresses and what their roles are")
	return nil
}
...
func checkInvalidSubnetNames(name string) error {
	...
	// this is currently exactly the same code as in luxd/vms/platformvm/create_chain_tx.go
	for _, r := range name {
