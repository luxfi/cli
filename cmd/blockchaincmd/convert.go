...
import (
	...
	"github.com/luxfi/cli/v2/v2/pkg/blockchain"
	...
	"github.com/luxfi/cli/v2/v2/pkg/localnet"
	...
	"github.com/luxfi/cli/v2/v2/pkg/signatureaggregator"
	...
	"github.com/luxfi/cli/v2/v2/pkg/validatormanager"
	blockchainSDK "github.com/luxfi/cli/v2/v2/sdk/blockchain"
	"github.com/luxfi/cli/v2/v2/sdk/evm"
	validatormanagerSDK "github.com/luxfi/cli/v2/v2/sdk/validatormanager"
...
Sovereign L1s require bootstrap validators. lux blockchain convert command gives the option of: 
...
func convertBlockchain(cmd *cobra.Command, args []string) error {
	...
	deployBalance := uint64(convertFlags.BootstrapValidatorFlags.DeployBalanceLUX * float64(units.Lux))
...
func printSuccessfulConvertOnlyOutput(blockchainName, subnetID string, generateNodeID bool) {
	...
	if generateNodeID {
		ux.Logger.PrintToUser("- Create the corresponding Lux node(s) with the provided Node ID and BLS Info")
	}
	ux.Logger.PrintToUser("- Have the Lux node(s) track the blockchain")
	ux.Logger.PrintToUser("- Call `lux contract initValidatorManager %s`", blockchainName)
	ux.Logger.PrintToUser("==================================================")
	if generateNodeID {
		ux.Logger.PrintToUser("To create the Lux node(s) with the provided Node ID and BLS Info:")
	...
	ux.Logger.PrintToUser("- Set '%s' as the value for 'track-subnets' configuration in ~/.luxd/config.json", subnetID)
