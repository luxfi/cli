...
func setWeight(_ *cobra.Command, args []string) error {
	...
	return CallAddValidator(
		...
		pop,
		newWeight,
		float64(balance)/float64(units.Lux),
...
func changeWeightACP99(
	...
	ux.Logger.PrintToUser("%s", luxlog.Yellow.Wrap("Validator manager owner %s pays for the initialization of the validator's weight change (Blockchain gas token)"), validatorManagerOwner)
	...
	ux.Logger.PrintToUser("%s", luxlog.Yellow.Wrap("RPC Endpoint: %s"), changeWeightFlags.RPC)
	...
	if newWeight != 0 {
		...
		if validatorInfo.Weight == newWeight {
			ux.Logger.PrintToUser("%s", luxlog.LightBlue.Wrap("The new Weight was already set on the P-Chain. Proceeding to the next step"))
			skipPChain = true
		}
	}
	if !skipPChain {
		...
		//		return err
		//	}
		//	ux.Logger.PrintToUser("%s", luxlog.LightBlue.Wrap("The Weight was already set to 0 on the P-Chain. Proceeding to the next step"))
		// } else {
