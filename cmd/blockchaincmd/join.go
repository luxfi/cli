...
func joinCmd(_ *cobra.Command, args []string) error {
	...
	if luxdConfigPath == "" {
		...
		if luxdConfigPath != "" {
			ux.Logger.PrintToUser("%s", luxlog.Bold.Wrap(luxlog.Green.Wrap(fmt.Sprintf("Found a config file at %s", luxdConfigPath))))
	...
	if pluginDir == "" {
		...
		if pluginDir != "" {
			ux.Logger.PrintToUser("%s", luxlog.Bold.Wrap(luxlog.Green.Wrap(fmt.Sprintf("Found the VM plugin directory at %s", pluginDir))))
