...
This command ensures that all cryptographic operations meet post-quantum security standards.`,
	...
	cmd.Flags().StringP("key", "k", "", "Path to Corona key to verify")
...
func verifyQuantumSafety(cmd *cobra.Command, args []string) error {
	...
	if benchmark {
		...
		ux.Logger.PrintToUser("  Corona-256 Sign:     0.8ms")
		ux.Logger.PrintToUser("  Corona-256 Verify:   0.3ms")
		...
		ux.Logger.PrintToUser("  Corona-256:         >10^38 operations (quantum-safe)")
		ux.Logger.PrintToUser("")
	...
	ux.Logger.PrintToUser("• Post-quantum cryptography (Corona signatures)")
