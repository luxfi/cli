...
func generateKeys(cmd *cobra.Command, args []string) error {
	...
	for i := 0; i < count; i++ {
		...
		if err := app.WriteFile(publicKeyPath, []byte(fmt.Sprintf("CORONA_PUBLIC_KEY_%d", i+1))); err != nil {
			return fmt.Errorf("failed to write public key: %w", err)
		}

		if err := app.WriteFile(privateKeyPath, []byte(fmt.Sprintf("CORONA_PRIVATE_KEY_%d", i+1))); err != nil {
