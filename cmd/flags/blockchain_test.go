...
import (
	...
	"github.com/luxfi/cli/v2/v2/pkg/prompts/mocks"
...
func TestValidateRPC(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			rpcURL:         "https://api.lux.network:9650",
			cmdName:        "testcmd",
			args:           []string{},
			wantError:      false,
			shouldPrompt:   false,
			expectedRPCURL: "https://api.lux.network:9650",
		},
		{
		...
		// Command-specific test cases
		{
			...
			args:           []string{},
			promptURL:      "https://api.lux.network:9650",
			promptError:    nil,
			wantError:      false,
			shouldPrompt:   true,
			expectedRPCURL: "https://api.lux.network:9650",
