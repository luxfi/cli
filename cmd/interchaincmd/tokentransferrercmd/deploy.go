...
import (
	...
	cmdflags "github.com/luxfi/cli/v2/v2/cmd/flags"
...
func NewDeployCmd() *cobra.Command {
	...
	cmd.Flags().StringVar(&deployFlags.version, "version", constants.WarpVersion, "tag/branch/commit of Lux Warp to be used")
...
func CallDeploy(_ []string, flags DeployFlags) error {
	...
	if flags.homeFlags.homeAddress == "" && flags.homeFlags.erc20Address == "" && !flags.homeFlags.native {
		...
		popularOption := "A popular token (e.g. WLUX, USDC, ...) (recommended)"
		...
		for {
			...
			case explainOption:
				ux.Logger.PrintToUser("An Lux Warp Transfer consists of one Home and at least one but possibly many Remotes.")
		...
		if !remoteManagedMinterAdmin {
			...
			if !remoteManagedMinterManager {
				if remoteMinterAdminFound {
					ux.Logger.PrintToUser("no managed key found for native minter admin %s on %s. add a CLI key for it using 'lux key create --file'", remoteMinterAdminAddress, remoteBlockchainDesc)
				}
				return fmt.Errorf("no managed key found for native minter manager %s on %s. add a CLI key for it using 'lux key create --file'", remoteMinterManagerAddress, remoteBlockchainDesc)
			}
		} else {
