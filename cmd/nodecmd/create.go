...
import (
	...
	"github.com/luxfi/cli/v2/v2/pkg/dependencies"
	...
	"github.com/luxfi/cli/v2/v2/pkg/docker"
	"github.com/luxfi/cli/v2/v2/pkg/metrics"
...
will apply to all nodes in the cluster`,
	...
	cmd.Flags().StringVar(&useLuxgoVersionFromSubnet, "luxd-version-from-subnet", "", "install latest luxd version, that is compatible with the given subnet, on node/s")
...
func preCreateChecks(clusterName string, network models.Network) error {
	if useCustomLuxgoVersion != "" || useLuxgoVersionFromSubnet != "" {
		...
		useLatestLuxgoReleaseVersion = false
		useLatestLuxgoPreReleaseVersion = false
	}
	if !flags.EnsureMutuallyExclusive([]bool{useLatestLuxgoReleaseVersion, useLatestLuxgoPreReleaseVersion}) {
		return fmt.Errorf("latest luxd released version, latest luxd pre-released version are mutually exclusive options")
	}
	if !flags.EnsureMutuallyExclusive([]bool{useLuxgoVersionFromSubnet != "", useCustomLuxgoVersion != ""}) {
		return fmt.Errorf("custom luxd version and luxd version based on given subnet, are mutually exclusive options")
	}
	if useAWS && useGCP {
	...
	if utils.IsE2E() {
		...
		if cloudService == constants.AWSCloudService {
			...
			for zone, numNodes := range numNodesMap {
				...
				if addMonitoring {
					prefix, err := defaultLuxCLIPrefix("")
					if err != nil {
						return err
					}
...
// CreateClusterNodeConfig creates node config and save it in .lux-cli/nodes/{instanceID}
// also creates cluster config in .lux-cli/nodes storing various key pair and security group info for all clusters
...
func setCloudService() (string, error) {
	...
	txt := "Which cloud service would you like to launch your Lux Node(s) in?"
...
func setCloudInstanceType(cloudService string) (string, error) {
	...
	if nodeType == "" {
		...
		if nodeTypeStr == customNodeType {
			nodeTypeStr, err = app.Prompt.CaptureString("What instance type would you like to use? Please refer to https://docs.lux.network/nodes/run/node-manually#hardware-and-os-requirements for minimum hardware requirements")
			if err != nil {
...
func requestCloudAuth(cloudName string) error {
	ux.Logger.PrintToUser("Do you authorize Lux-CLI to access your %s account?", cloudName)
	ux.Logger.PrintToUser("By clicking yes, you are authorizing Lux-CLI to:")
	...
	yes, err := app.Prompt.CaptureYesNo(fmt.Sprintf("I authorize Lux-CLI to access my %s account", cloudName))
	if err != nil {
		return err
	}
	...
	if !yes {
		return fmt.Errorf("user did not give authorization to Lux-CLI to access %s account", cloudName)
	}
	return nil
}
...
// defaultLuxCLIPrefix returns the default Lux CLI prefix.
func defaultLuxCLIPrefix(region string) (string, error) {
	usr, err := user.Current()
	if err != nil {
		return "", err
	}
	if region == "" {
		return usr.Username + constants.LuxCLISuffix, nil
	}
	return usr.Username + "-" + region + constants.LuxCLISuffix, nil
}
