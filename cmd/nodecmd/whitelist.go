...
func GrantAccessToIPinAWS(awsProfile string, region string, sgName string, userIPAddress string) error {
	...
	if ipInHTTP {
		...
		if err := ec2Svc.AddSecurityGroupRule(*sg.GroupId, "ingress", "tcp", userIPAddress, constants.LuxGoAPIPort); err != nil {
	...
	if ipInGrafana {
		...
		if err := ec2Svc.AddSecurityGroupRule(*sg.GroupId, "ingress", "tcp", userIPAddress, constants.LuxGoGrafanaPort); err != nil {
...
func GrantAccessToIPinGCP(userIPAddress string) error {
	...
	if _, err = gcpCloud.SetFirewallRule(userIPAddress, fmt.Sprintf("%s-%s", networkName, strings.ReplaceAll(userIPAddress, ".", "")), networkName, []string{strconv.Itoa(constants.SSHTCPPort), strconv.Itoa(constants.LuxGoAPIPort), strconv.Itoa(constants.LuxGoGrafanaPort)}); err != nil {
