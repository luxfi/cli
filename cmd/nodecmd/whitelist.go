...
func GrantAccessToIPinAWS(awsProfile string, region string, sgName string, userIPAddress string) error {
	...
	if ipInHTTP {
		...
		if err := ec2Svc.AddSecurityGroupRule(*sg.GroupId, "ingress", "tcp", userIPAddress, constants.LuxdAPIPort); err != nil {
	...
	if ipInGrafana {
		...
		if err := ec2Svc.AddSecurityGroupRule(*sg.GroupId, "ingress", "tcp", userIPAddress, constants.LuxdGrafanaPort); err != nil {
...
func GrantAccessToIPinGCP(userIPAddress string) error {
	...
	if _, err = gcpCloud.SetFirewallRule(userIPAddress, fmt.Sprintf("%s-%s", networkName, strings.ReplaceAll(userIPAddress, ".", "")), networkName, []string{strconv.Itoa(constants.SSHTCPPort), strconv.Itoa(constants.LuxdAPIPort), strconv.Itoa(constants.LuxdGrafanaPort)}); err != nil {
