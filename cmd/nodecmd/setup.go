...
func newSetupCmd() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "setup",
		Short: "Sets up a new Lux Node on remote server",
		Long: `The node setup command installs Lux Go on specified remote servers. 
...
func setup(hosts []*models.Host, luxdVersion string, network models.Network) error {
	...
	ux.Logger.PrintToUser("Setting up Lux node(s)...")
	wg := sync.WaitGroup{}
