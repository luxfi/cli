...
func syncSubnet(_ *cobra.Command, args []string) error {
	...
	ux.Logger.PrintToUser(fmt.Sprintf("Check node subnet syncing status with lux node status %s --subnet %s", clusterName, subnetName))
	return nil
}

// trackSubnet exports deployed subnet in user's local machine to cloud server and calls node to
// start tracking the specified subnet (similar to lux subnet join <subnetName> command)
func trackSubnet(
	hosts []*models.Host,
	subnetName string,
