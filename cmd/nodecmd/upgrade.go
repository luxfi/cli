...
func newUpgradeCmd() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "upgrade",
		Short: "(ALPHA Warning) Update luxd or VM version for all node in a cluster",
...
func upgrade(_ *cobra.Command, args []string) error {
	...
	for host, upgradeInfo := range toUpgradeNodesMap {
		if upgradeInfo.LuxdVersion != "" {
			spinner := spinSession.SpinToUser(utils.ScriptLog(host.NodeID, "Upgrading luxd to version %s...", upgradeInfo.LuxdVersion))
		...
		if upgradeInfo.SubnetEVMVersion != "" {
			...
			spinner := spinSession.SpinToUser(utils.ScriptLog(host.NodeID, "Upgrading SubnetEVM to version %s...", upgradeInfo.SubnetEVMVersion))
			...
			for _, vmID := range upgradeInfo.SubnetEVMIDsToUpgrade {
				subnetEVMBinaryPath := fmt.Sprintf("/home/ubuntu/.luxd/plugins/%s", vmID)
...
func getNodesUpgradeInfo(hosts []*models.Host) (map[*models.Host]nodeUpgradeInfo, error) {
	...
	if wgResults.HasErrors() {
		return nil, fmt.Errorf("failed to get luxd version for node(s) %s", wgResults.GetErrorHostMap())
	}
