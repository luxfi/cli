...
The ring signature uses your Corona key (secp256k1) from ~/.lux/keys/<name>/rt/
...
func runRingSign(_ *cobra.Command, args []string) error {
	...
	switch scheme {
	case ring.LSAG:
		signer, err = ring.NewLSAGSignerFromPrivateKey(keySet.CoronaPrivateKey)
...
func runRingSchemes(_ *cobra.Command, _ []string) error {
	...
	ux.Logger.PrintToUser("    - Uses Corona keys from ~/.lux/keys/<name>/rt/")
...
func buildRing(signerName string, ringNames []string, scheme ring.Scheme, signerKeySet *key.HDKeySet) ([][]byte, int, error) {
	...
	for i, name := range ringNames {
		...
		if name == signerName {
			...
			switch scheme {
			case ring.LSAG:
				pubKey = signerKeySet.CoronaPublicKey
			case ring.LatticeLSAG:
...
func buildRingFromNames(ringNames []string, scheme ring.Scheme) ([][]byte, error) {
	...
	for i, name := range ringNames {
		...
		switch scheme {
		case ring.LSAG:
			ringPubKeys[i] = ks.CoronaPublicKey
		case ring.LatticeLSAG:
