...
func TestFindDefaultFiles(t *testing.T) {
	...
	origVar := os.Getenv(config.LuxdDataDirVar)
	err = os.Setenv(config.LuxdDataDirVar, existingDataDir)
	require.NoError(err)
	defer func() {
		err = os.Setenv(config.LuxdDataDirVar, origVar)
		require.NoError(err)
