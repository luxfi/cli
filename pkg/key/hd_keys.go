...
const (
	...
	CoronaKeyDir = "rt"       // Corona keys for ring signatures
...
func DeriveAllKeys(name, mnemonic string) (*HDKeySet, error) {
	...
	// Derive Corona key
	keySet.CoronaPrivateKey, err = deriveKeyFromSeed(seed, DomainCorona, 32)
	if err != nil {
...
func SaveKeySet(keySet *HDKeySet) error {
	...
	if err := os.WriteFile(filepath.Join(rtDir, PrivateKeyFile), []byte(hex.EncodeToString(keySet.CoronaPrivateKey)), 0600); err != nil {
		return fmt.Errorf("failed to save Corona private key: %w", err)
	}
	if err := os.WriteFile(filepath.Join(rtDir, PublicKeyFile), []byte(hex.EncodeToString(keySet.CoronaPublicKey)), 0644); err != nil {
...
func LoadKeySet(name string) (*HDKeySet, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("failed to load Corona private key: %w", err)
	}
	keySet.CoronaPrivateKey, err = hex.DecodeString(string(rtPrivHex))
	if err != nil {
		return nil, fmt.Errorf("failed to decode Corona private key: %w", err)
	}
	rtPubHex, err := os.ReadFile(filepath.Join(rtDir, PublicKeyFile))
	if err != nil {
		return nil, fmt.Errorf("failed to load Corona public key: %w", err)
	}
	keySet.CoronaPublicKey, err = hex.DecodeString(string(rtPubHex))
	if err != nil {
		return nil, fmt.Errorf("failed to decode Corona public key: %w", err)
	}
