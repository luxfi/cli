...
func InstallSignatureAggregator(app *application.Lux, version *string) (string, error) {
...
func StartSignatureAggregator(app *application.Lux, network models.Network, configPath string, logFile string, logger logging.Logger, version string, signatureAggregatorEndpoint string) (int, error) {
...
func CreateSignatureAggregatorInstance(app *application.Lux, subnetIDStr string, network models.Network, extraAggregatorPeers []info.Peer, aggregatorLogger logging.Logger, version string) error {
...
func getCurrentSignatureAggregatorProcessDetails(app *application.Lux, network models.Network) (*signatureAggregatorRunFile, error) {
	runFilePath := app.GetLocalSignatureAggregatorRunPath(network.Kind)
...
func stopSignatureAggregator(app *application.Lux, network models.Network) error {
	runFilePath := app.GetLocalSignatureAggregatorRunPath(network.Kind)
...
func restartSignatureAggregator(app *application.Lux, network models.Network, configPath string, logger logging.Logger) error {
...
func UpdateSignatureAggregatorPeers(app *application.Lux, network models.Network, extraAggregatorPeers []info.Peer, logger logging.Logger) error {
	// Get the config path
	configPath := filepath.Join(app.GetSignatureAggregatorRunDir(network.Kind), "config.json")
