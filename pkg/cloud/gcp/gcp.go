...
func (c *GcpCloud) SetupNetwork(ipAddress, networkName string) (*compute.Network, error) {
	...
	if _, err := c.SetFirewallRule("0.0.0.0/0",
		...
		networkName,
		[]string{strconv.Itoa(constants.LuxdP2PPort), strconv.Itoa(constants.LuxdLokiPort)}); err != nil {
		return nil, err
	}
	if _, err := c.SetFirewallRule(ipAddress,
		...
		networkName,
		[]string{
			strconv.Itoa(constants.SSHTCPPort), strconv.Itoa(constants.LuxdAPIPort),
...
func (c *GcpCloud) DestroyGCPNode(nodeConfig models.NodeConfig, clusterName string) error {
	...
	ux.Logger.PrintToUser("Destroying node instance %s in cluster %s...", nodeConfig.NodeID, clusterName)
	...
	if nodeConfig.UseStaticIP {
		ux.Logger.PrintToUser("Releasing static IP address %s ...", nodeConfig.ElasticIP)
