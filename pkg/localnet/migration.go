	...
	if cli != nil {
		...
		if status != nil && status.ClusterInfo != nil {
			...
			if _, err := cli.Stop(ctx); err != nil {
				return fmt.Errorf("failed to stop luxd: %w", err)
			}
		}
...
func migrateCluster(
	...
	for _, nodeConfig := range config.NodeConfigs {
		...
		httpPort, err := utils.GetJSONKey[float64](nodeConfig.Flags, luxdconfig.HTTPPortKey)
		if err != nil {
			return fmt.Errorf("failure reading legacy local network conf: %w", err)
		}
		stakingPort, err := utils.GetJSONKey[float64](nodeConfig.Flags, luxdconfig.StakingPortKey)
		if err != nil {
			return fmt.Errorf("failure reading legacy local network conf: %w", err)
		}
	...
	defaultFlags[luxdconfig.PartialSyncPrimaryNetworkKey] = true
	defaultFlags[luxdconfig.NetworkAllowPrivateIPsKey] = true
	defaultFlags[luxdconfig.IndexEnabledKey] = false
	defaultFlags[luxdconfig.IndexAllowIncompleteKey] = true
