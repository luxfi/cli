...
// SaveNetworkState saves the current P-Chain state to disk.
...
// ClearNetworkState removes all saved network state.
...
// getPersistedTrackingConfig reads tracked subnets and dev mode from persisted state.
func getPersistedTrackingConfig(networkDir string) ([]string, bool, error) {
	statePath := networkDir + "/" + networkStateFilename
	bs, err := os.ReadFile(statePath)
	if err != nil {
	...
	var stateData NetworkStateData
	if err := json.Unmarshal(bs, &stateData); err != nil {
		return nil, false, err
	}
	return stateData.TrackedSubnets, stateData.DevMode, nil
}
