...
import (
	...
	api "github.com/luxdefi/luxgo/api"

	ids "github.com/luxdefi/luxgo/ids"

	mock "github.com/stretchr/testify/mock"

	platformvm "github.com/luxdefi/luxgo/vms/platformvm"

	rpc "github.com/luxdefi/luxgo/utils/rpc"

	secp256k1 "github.com/luxdefi/luxgo/utils/crypto/secp256k1"

	status "github.com/luxdefi/luxgo/vms/platformvm/status"

	time "time"

	validators "github.com/luxdefi/luxgo/snow/validators"
)
...
// ExportLUX provides a mock function with given fields: ctx, user, from, changeAddr, to, toChainIDAlias, amount, options
func (_m *PClient) ExportLUX(ctx context.Context, user api.UserPass, from []ids.ShortID, changeAddr ids.ShortID, to ids.ShortID, toChainIDAlias string, amount uint64, options ...rpc.Option) (ids.ID, error) {
	_va := make([]interface{}, len(options))
	for _i := range options {
		_va[_i] = options[_i]
	}
...
// ImportLUX provides a mock function with given fields: ctx, user, from, changeAddr, to, sourceChain, options
func (_m *PClient) ImportLUX(ctx context.Context, user api.UserPass, from []ids.ShortID, changeAddr ids.ShortID, to ids.ShortID, sourceChain string, options ...rpc.Option) (ids.ID, error) {
	_va := make([]interface{}, len(options))
	for _i := range options {
		_va[_i] = options[_i]
	}
