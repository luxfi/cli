...
**Flags:**
- `--num-nodes` - Number of nodes to start (default: 3)
...
# Start from snapshot
lux network start --snapshot-name=mysnapshot
...
**Flags:**
- `--snapshot-name` - Save network state as snapshot
...
lux network stop --snapshot-name=mysnapshot
```
...
```bash
# During network stop
lux network stop --snapshot-name=my-test-state
...
**Load Snapshot:**
```bash
lux network start --snapshot-name=my-test-state
```
...
}' -H 'content-type:application/json' http://127.0.0.1:9630/v1/health/ops/health
...
# Access metrics
curl http://127.0.0.1:9630/v1/metrics
```
...
lux network stop --snapshot-name=test-state
```
...
# Verify endpoints
curl http://127.0.0.1:9630/v1/health/ops/health
