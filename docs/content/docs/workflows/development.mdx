...
lux blockchain create source-chain --vm=evm
lux blockchain deploy source-chain --local

# 2. Deploy destination chain
lux blockchain create dest-chain --vm=evm
...
# Start test network
lux network start --snapshot-name=clean-state
...
lux network stop --snapshot-name=integration-passed
```
