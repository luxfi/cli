...
var _ = ginkgo.Describe("[Upgrade expect network failure SOV]", ginkgo.Ordered, func() {
	...
	ginkgo.It("fails on stopped network SOV", func() {
		commands.CreateEVMConfigSOV(chainName, utils.EVMGenesisPath)
...
var _ = ginkgo.Describe("[Upgrade local network SOV]", ginkgo.Ordered, func() {
	...
	ginkgo.It("fails on undeployed chain SOV", func() {
		commands.CreateEVMConfigSOV(chainName, utils.EVMGenesisPath)
	...
	ginkgo.It("can create and apply to locally running chain SOV", func() {
		commands.CreateEVMConfigSOV(chainName, utils.EVMGenesisPath)
	...
	ginkgo.It("can't upgrade transactionAllowList precompile because admin address doesn't have enough token SOV", func() {
		...
		commands.DeployChainLocallySOV(chainName)
	...
	ginkgo.It("can create and update future SOV", func() {
		...
		commands.CreateEVMConfigWithVersionSOV(chainName, utils.EVMGenesisPath, evmVersion1)
	...
	ginkgo.It("upgrade custom vm local deployment SOV", func() {
		...
		// create and deploy
		commands.CreateCustomVMConfigSOV(chainName, utils.EVMGenesisPath, customVMPath1)
	...
	ginkgo.It("can upgrade evm on public deployment SOV", func() {
		...
		commands.CreateEVMConfigWithVersionSOV(chainName, utils.EVMGenesisPath, binaryToVersion[utils.SoloEVMKey1])
