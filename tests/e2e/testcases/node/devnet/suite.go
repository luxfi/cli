...
var _ = ginkgo.Describe("[Node devnet]", func() {
	ginkgo.It("can create a node", func() {
		outputB, err := ansi.Strip([]byte(commands.NodeDevnet(luxdVersion, NumNodes, NumAPINodes)))
	...
	ginkgo.It("installs and runs luxd", func() {
		...
		gomega.Expect(luxdProcess).To(gomega.ContainSubstring("avaplatform/luxd:"))
	})
	ginkgo.It("configured luxd", func() {
		...
		gomega.Expect(luxdConfig).To(gomega.ContainSubstring("\"genesis-file\": \"/.luxd/configs/genesis.json\""))
		gomega.Expect(luxdConfig).To(gomega.ContainSubstring("\"network-id\": \"network-1338\""))
		gomega.Expect(luxdConfig).To(gomega.ContainSubstring("\"public-ip\": \"" + constants.E2ENetworkPrefix))
