...
var _ = ginkgo.Describe("[Node create]", func() {
	...
	ginkgo.It("creates cluster config", func() {
		...
		gomega.Expect(err).Should(gomega.BeNil())
		homeDir := usr.HomeDir
		content, err := os.ReadFile(filepath.Join(homeDir, constants.E2EBaseDirName, nodesRelativePath, constants.ClustersConfigFileName)) //nolint:gosec // G304: Test code reading from test directories
		gomega.Expect(err).Should(gomega.BeNil())
	...
	ginkgo.It("creates node config", func() {
		...
		gomega.Expect(err).Should(gomega.BeNil())
		homeDir := usr.HomeDir
		content, err := os.ReadFile(filepath.Join(homeDir, constants.E2EBaseDirName, nodesRelativePath, hostName, "node_cloud_config.json")) //nolint:gosec // G304: Test code reading from test directories
		gomega.Expect(err).Should(gomega.BeNil())
	...
	ginkgo.It("installs and runs luxd", func() {
	...
	ginkgo.It("logged operations", func() {
		ls := commands.NodeSSH(constants.E2EClusterName, "ls -l /home/ubuntu/.luxd/*")
		fmt.Println(ls)
	...
	ginkgo.It("can export cluster config", func() {
		...
		gomega.Expect(output).To(gomega.ContainSubstring(fmt.Sprintf("to /home/runner/work/lux-cli/lux-cli/%s", exportFileName)))
