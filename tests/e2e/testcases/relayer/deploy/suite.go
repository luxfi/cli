	...
	ginkgo.Context("With valid input", func() {
		ginkgo.Context("With non SOV subnet", func() {
			...
			ginkgo.It("should deploy the relayer on c-chain and subnet with funding it from another key", func() {
				...
				gomega.Expect(err).Should(gomega.BeNil())
				gomega.Expect(key2BalanceCchain).Should(gomega.Equal(90 * units.Lux))
				...
				gomega.Expect(err).Should(gomega.BeNil())
				gomega.Expect(key2BalanceSubnet).Should(gomega.Equal(50 * units.Lux))
				...
				gomega.Expect(err).Should(gomega.BeNil())
				gomega.Expect(keyBalanceCchain).Should(gomega.Equal(10*units.Lux - cchainFee))
				...
				gomega.Expect(err).Should(gomega.BeNil())
				gomega.Expect(keyBalanceSubnet).Should(gomega.Equal(50*units.Lux - subnetFee))
			})
		})
